...
function Transfer(a, b) {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        if (
            ((water[b][i] != "transparent" &&
                water[b][i + 1] == "transparent") ||
    ...
    for (let i = 3; i >= 0; i--) {
        if (
            (water[a][i] == p[0] || water[a][i] == "transparent") &&
            count < q[2]
        ) {
            if (water[a][i] == p[0]) {
                count++;
...
function WaterDec(p, a, count) {
    ...
    setTimeout(function () {
        document.getElementById("white-bg").style.height =
            count * 30 + 1 + "px";
    }, 50);
...
function WaterInc(p, q, b, count) {
    ...
    setTimeout(function () {
        document.getElementById("colorful-bg").style.height =
            count * 30 + 1 + "px";
